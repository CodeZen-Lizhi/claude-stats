...
struct CLIEnvironmentCheckerTests {
    ...
    func versionParserExtractsSemverAndFallsBack() {
        #expect(CLIEnvironmentChecker.extractVersion(from: "codex 2.1.43\n") == "2.1.43")
    ...
    func toolStatusOnlyMarksSemverBehindInstallsAsOutdated() {
        let behind = CLIToolStatus(cli: .codex, command: "codex", version: "2.1.43", latestVersion: "2.1.44", error: nil, diagnostic: nil, envType: .macOS, executablePath: nil)
        let current = CLIToolStatus(cli: .codex, command: "codex", version: "2.1.44", latestVersion: "2.1.44", error: nil, diagnostic: nil, envType: .macOS, executablePath: nil)
        let ahead = CLIToolStatus(cli: .codex, command: "codex", version: "2.1.45", latestVersion: "2.1.44", error: nil, diagnostic: nil, envType: .macOS, executablePath: nil)
        let raw = CLIToolStatus(cli: .codex, command: "codex", version: "nightly channel", latestVersion: "2.1.44", error: nil, diagnostic: nil, envType: .macOS, executablePath: nil)
    ...
    func localDetectionFallsBackToScannedPaths() async throws {
        ...
        let executable = dir.appendingPathComponent(".volta/bin/codex", isDirectory: false)
        ...
        let runner: CLIEnvironmentChecker.ProcessRunner = { invocation in
            if invocation.executablePath == executable.path {
                return CLIVersionProcessResult(exitCode: 0, stdout: "Codex 2.1.43\n", stderr: "", launchError: nil, timedOut: false)
            }
        ...
        let checker = CLIEnvironmentChecker(
            ...
            latestVersionFetcher: { cli in cli == .codex ? "2.1.44" : nil }
        )
    ...
    func environmentScanFindsProcessAndShellFileConflicts() async throws {
        ...
        #expect(conflicts.contains { $0.cli == .codex && $0.varName == "OPENAI_AUTH_TOKEN" && $0.sourceType == .process })
        #expect(conflicts.contains { $0.cli == .codex && $0.varName == "OPENAI_BASE_URL" && $0.lineNumber == 1 && $0.isDeletable })
    ...
    func deletingConflictsBacksUpFilesAndRemovesExactLines() async throws {
        ...
        try TempDir.write(
            """
            export OPENAI_AUTH_TOKEN=sk-openai-auth
            KEEP_ME=1
            OPENAI_API_KEY=sk-openai
        ...
        #expect(!updated.contains("OPENAI_AUTH_TOKEN"))
    ...
    func loadIfNeededOnlyLoadsOnceAndRefreshForcesRerun() async throws {
        let checker = CountingCLIEnvironmentChecker(
            snapshot: CLIEnvironmentSnapshot(
                statuses: [CLIToolStatus(cli: .codex, command: "codex", version: "2.1.43", latestVersion: nil, error: nil, diagnostic: nil, envType: .macOS, executablePath: nil)],
                conflicts: []
            )
        )
