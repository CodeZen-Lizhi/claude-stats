...
struct GitChangedFileRowModel: Identifiable, Hashable, Sendable {
    ...
    init(commitFile file: CommitFileChange) {
        ...
        statusBadges = []
        openPath = file.path
        helpText = L10n.format("git.changed_files.open_diff", defaultValue: "Open diff for %@", file.path)
    ...
    private static func statusBadges(for change: GitWorkingTreeChange) -> [Badge] {
        ...
        if change.isStaged {
            badges.append(Badge(
                label: L10n.string("git.changed_files.staged", defaultValue: "staged"),
                help: L10n.string("git.changed_files.staged.help", defaultValue: "Staged"),
                tone: .muted
            ))
        }
        if change.isUnstaged {
            badges.append(Badge(
                label: L10n.string("git.changed_files.unstaged", defaultValue: "unstaged"),
                help: L10n.string("git.changed_files.unstaged.help", defaultValue: "Unstaged"),
                tone: .muted
            ))
        }
        return badges
