...
protocol OpenAIStatusUptimeCaching: Sendable {
...
struct OpenAIStatusUptimeCache: OpenAIStatusUptimeCaching {
    ...
    func read(ttl: TimeInterval = Self.defaultTTL, now: Date = .now) -> (snapshot: OpenAIStatusUptimeSnapshot, isStale: Bool)? {
        ...
        do {
            ...
            let snapshot = try decoder.decode(OpenAIStatusUptimeSnapshot.self, from: data)
            return (snapshot, now.timeIntervalSince(snapshot.fetchedAt) > ttl)
        } catch {
            Log.app.error("OpenAI Status uptime cache decode failed: \(error.localizedDescription, privacy: .public)")
            return nil
        }
    }
